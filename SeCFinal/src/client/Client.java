...
public class Client {
	...
	public Client() {
		filePath = new String("repositorio");
	}
	...
	public static void addPhotos(String operation,String photos) throws IOException, ClassNotFoundException {
		...
		String[] op = {operation};
		handlerTCP.send(concat(op,fileNames));
		String currFile;

		for (String s : fileNames) {
			for (File f : files) {
				// Compara fotos que se pretende enviar com as que estao 
				//no repositorio local
				//check if the photo has an extention
				currFile = f.getName();
